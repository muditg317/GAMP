...
class SelectiveGaze_CL_ActionNet(MoGaS_Gazed_ActionNet):
  def __init__(self, *,
    ...
    self.cgl_linear_as_conv = nn.Linear(np.prod(self.lin_in_shape), np.prod(self.lin_in_shape))
    self.cgl_softmax = nn.Softmax()
  ...
  def forward(self, x, gaze_true):
    ...
    cgl_out = self.cgl_linear_as_conv(x.flatten(start_dim=1))
    cgl_out = self.cgl_softmax(cgl_out)

    return x, cgl_out

  def loss_fn(self, loss_, acts, targets, gaze_true:torch.Tensor=None, cgl_out:torch.Tensor=None):
    ce_loss = super().loss_fn(loss_, acts, targets)
    cgl_loss = compute_coverage_loss(cgl_out, gaze_true.squeeze(1))
    return ce_loss*(1-self.coverage_loss_weight) + cgl_loss*self.coverage_loss_weight
