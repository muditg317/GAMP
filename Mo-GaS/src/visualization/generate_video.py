...
game_run_frames = OrderedDict({
    ...
    for entry in os.listdir(
        os.path.join(os.path.join(INTERIM_DATA_DIR, game), game_run))
...
game_run_data_mod_df.to_csv('{}/processed_game_data.csv'.format(proc_writ_dir))
...
for fid in tqdm(frame_ids.index):
    ...
    if os.path.exists(os.path.join(img_writ_dir, game_run_frames[fid + 1])):
        continue
...
imtype = imtypes_[1]
if imtype == 'diff':
    img_writ_dir = img_diff_writ_dir
frame_rate = 20
ffmpeg_arg_string = 'ffmpeg -y -framerate {} -i {}/{}_%d.png {}/../out.mp4'.format(
    frame_rate, img_writ_dir, '_'.join(game_run.split('_')[1:3]), img_writ_dir)
