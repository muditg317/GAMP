...
class Dataset:
  def __init__(self, tar_fname, label_fname):
    ...
    preprocess_and_sanity_check(png_files, frame_ids)

    if not os.path.exists(temp_extract_dir+'/'+png_files[0].split('/')[0]) or len(os.listdir(temp_extract_dir+'/'+png_files[0].split('/')[0])) != len(png_files):
        if os.path.exists(temp_extract_dir+'/'+png_files[0].split('/')[0]):
            print(len(os.listdir(temp_extract_dir+'/'+png_files[0].split('/')[0])), len(png_files))
    ...
    for i in range(self.train_size):
        ...
        img = np.array(cv2.imread(png_fname), dtype=np.float32)
        img = preprocess(img)
        imgs[i] = copy.deepcopy(img)
        if i % 1000 == 0:
            print("%d/%d" % (i+1,self.train_size), end='\t'),
            sys.stdout.flush()
